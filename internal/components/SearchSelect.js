(function () {
  ...
  const searchSelect = document.querySelector(".search-select-container");
  const isMultiple = searchSelect.getAttribute("data-multiple");
  const hiddenInput = searchSelect.querySelector("input[type='hidden']");
  const searchInput = searchSelect.querySelector("input[type='search']");
  ...
  if (isMultiple) {
    selectedValuesContainer = searchSelect.querySelector(".selected-values");
  } else {
    arrow = searchSelect.querySelector(".arrow");
  }
  ...
  searchInput.addEventListener("keyup", () => {
    if (isMultiple) {
      ...
      searchInput.style.width = `${newWidth + 1}px`;
    }
