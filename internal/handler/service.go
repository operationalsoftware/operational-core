...
func (h *ServiceHandler) ResourceServicingPage(w http.ResponseWriter, r *http.Request) {
	...
	resources, count, err := h.servicesService.GetResourceServiceMetricStatuses(
		r.Context(),
		ctx.User.UserID,
		model.ResourceServiceMetricStatusesQuery{
...
func (h *ServiceHandler) UpdateResourceServiceNotes(w http.ResponseWriter, r *http.Request) {
	...
	resource, err := h.resourceService.GetResourceByID(
		r.Context(),
...
func (h *ServiceHandler) ResourceServicePage(w http.ResponseWriter, r *http.Request) {
	...
	if canManage {
		...
		resourceService.GalleryURL = h.galleryService.GenerateTempURL(resourceService.GalleryID, false)
	}
	breadcrumbContext, err := layout.EncodeBreadcrumbs([]layout.Breadcrumb{
		...
		{
			Title: fmt.Sprintf("Service of %s", resourceService.ResourceReference),
			URL:   fmt.Sprintf("/services/%d", resourceService.ResourceServiceID),
		},
	})
	if err == nil {
		resourceService.GalleryURL = appendQueryParams(resourceService.GalleryURL, map[string]string{
	...
	_ = serviceview.ResourceServicePage(&serviceview.ResourceServicePageProps{
		...
		CanManage:               canManage,
		CanDelete:               canManage && !hasNewerService,
